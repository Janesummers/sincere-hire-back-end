...
let getJobDetail = (req, resp) => {
  ...
  mysqlOpt.exec(
    `select job.*, comp.company_name, comp.size as company_size, comp.type as company_type, u.nickname as publisher_name, u.position as position, u.avatarUrl, (select job_id from collect where job_id = ?) as col_job_id
    ...
    mysqlOpt.formatParams(job_id, job_id),
    (res) => {
      resp.json(msgResult.msg(res))
    },
    e => {
